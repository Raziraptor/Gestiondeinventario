...
@purchasing_bp.route('/api/ordenes/<int:orden_id>/enviar-hd', methods=['POST'])
@login_required
@check_org_permission
def enviar_oc_homedepot(orden_id):
    ...
    integracion = ProveedorIntegracion.query.filter_by(
        proveedor_id=orden.proveedor_id, organizacion_id=org_id,
        tipo='homedepot', activo=True,
    ).first()
    if not integracion:
    ...
    items = [
        {'sku': d.producto.hd_sku or '', 'nombre': d.producto.nombre, 'cantidad': d.cantidad_solicitada or 1}
        for d in orden.detalles if d.producto
    ]
    if not items:
...
@purchasing_bp.route('/api/ordenes/<int:orden_id>/integracion-status', methods=['GET'])
@login_required
@check_org_permission
def integracion_status_oc(orden_id):
    ...
    if orden.integracion_resultado:
        try:
            ...
            pass
    return jsonify({'status': orden.integracion_status, 'resultado': resultado})
...
@purchasing_bp.route('/proyecto-oc/exportar.xlsx')
@login_required
@check_org_permission
@check_permission('perm_create_oc_proyecto')
def exportar_proyectos_oc_excel():
    ...
    if estado_f: query = query.filter(ProyectoOC.estado == estado_f)
    if mes_f:    query = query.filter(extract('month', ProyectoOC.fecha_creacion) == mes_f)
    if ano_f:    query = query.filter(extract('year',  ProyectoOC.fecha_creacion) == ano_f)
    ...
    estado_labels = {'borrador':'Borrador','enviada':'Enviada','recibida':'Recibida','cancelada':'Cancelada'}

    for i, poc in enumerate(proyectos, 2):
        fill = alt_fill if i % 2 == 0 else PatternFill()
        row  = [
            poc.id, poc.nombre_proyecto, estado_labels.get(poc.estado, poc.estado),
            poc.creador.username, poc.fecha_creacion.strftime('%d/%m/%Y'),
            ...
            len(poc.detalles), poc.costo_total,
        ]
        ws.append(row)
    ...
    for poc in proyectos:
        for d in poc.detalles:
            if d.producto_id and d.producto:
                tipo, nombre, sku = 'Catálogo', d.producto.nombre, d.producto.codigo
            else:
                tipo, nombre, sku = 'Externo', d.descripcion_nuevo or 'Sin descripción', '—'
            sub  = d.cantidad * d.costo_unitario
